...
{
    // Convert otocash:// to otocash:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("otocash://"))
    {
        uri.replace(0, 12, "otocash:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OTOCASH.lnk";
}
...
{
    return GetAutostartDir() / "OTOCASH.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("OTOCASH-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  OTOCASH-qt [" + tr("command-line options") + "]                     " + "\n";
