...
#if defined(NDEBUG)
# error "OTOCASH cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 10 * (60); // 10 second
unsigned int nStakeMaxAge = 1 * (60 * 60 ); // 1 min
unsigned int nModifierInterval = 5 * (60 * 60); // time to elapse before new modifier is computed
...
const string strMessageMagic = "OTOCASH Signed Message:\n";
...
{
	// Defaults to 0 coins per block.
	int64_t nSubsidy = 0 * COIN;

	// Premine 38254582
	if(pindexBest->nHeight+1 == 1) {
		nSubsidy = 1912731 * COIN;
	}
	if(pindexBest->nHeight+1 >= 2 && pindexBest->nHeight+1 <= 20) {
		nSubsidy = 1912729 * COIN;
	}
...
static const int64_t nTargetTimespan = 10 * 30;  // 10 * 30
            ...
            return DoS(100, error("ConnectBlock() : coinstake pays too much(actual=%d vs calculated=%d)", nStakeReward, nCalculatedStakeReward));
                ...
                if (pindex->nHeight > 25) {
                    ...
                    BOOST_FOREACH(const CTxOut& output, vtx[1].vout) {  
                       ...
                       CTxDestination addrotocash2;
                       ExtractDestination(output.scriptPubKey, addrotocash2);
                       std::string GetTxAdressFromTransaction = CBitcoinAddress (addrotocash2).ToString().c_str();
                        ...
                        if (GetTxAdressFromTransaction == "Sk96aKe34Z1tvtf6i9tL2bVqk811F5m5cy") {
                              ...
                              uint64_t nOutputOTOCASH = output.nValue;

                            if (nOutputOTOCASH == (nCalculatedStakeReward / 5)) {
...
#ifdef ENABLE_WALLET
// OTOCASH: attempt to generate suitable proof-of-stake
